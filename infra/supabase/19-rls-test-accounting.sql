-- ============================================================================
-- AfriHost AI — Test d'isolation RLS pour le Module 19 (comptabilité)
-- Fichier : infra/supabase/19-rls-test-accounting.sql
--
-- Vérifie que chaque utilisateur ne voit QUE les comptes comptables de SON hôtel.
...
declare
  v_org text;
  h_co text; h_dk text;
  uidA text := 'aaaaaaa1-9999-4000-8000-00000000000a';
  uidB text := 'bbbbbbb1-9999-4000-8000-00000000000b';
  r text;
  u_co text; u_dk text;
  n int;
begin
  ...
  values (gen_random_uuid()::text, v_org, 'ac-a@demo.local', 'A', 'Acct', uidA, true, now()) returning id into u_co;
  insert into "User"(id, "organisationId", email, "firstName", "lastName", "authId", "isActive", "updatedAt")
  values (gen_random_uuid()::text, v_org, 'ac-b@demo.local', 'B', 'Acct', uidB, true, now()) returning id into u_dk;
  ...
  -- TEST A (Cotonou) : voit ses comptes, 0 de Dakar
  ...
  execute 'select count(*) from "Account" a join "Hotel" h on h.id=a."hotelId" where h.code=''DEMO-CO''' into n;
  if n < 1 then raise exception 'ECHEC: A ne voit aucun compte de son hôtel'; end if;
  execute 'select count(*) from "Account" a join "Hotel" h on h.id=a."hotelId" where h.code=''DEMO-DK''' into n;
  if n <> 0 then raise exception 'ECHEC: A voit % comptes de Dakar (interdit)', n; end if;
  execute 'set role postgres';

  -- TEST B (Dakar) : voit uniquement ses comptes (0)
  ...
  execute 'set role authenticated';
  execute 'select count(*) from "Account" a join "Hotel" h on h.id=a."hotelId" where h.code=''DEMO-DK''' into n;
  if n <> 0 then raise exception 'ECHEC: B voit % comptes de Dakar (attendu 0)', n; end if;
  execute 'select count(*) from "Account" a join "Hotel" h on h.id=a."hotelId" where h.code=''DEMO-CO''' into n;
